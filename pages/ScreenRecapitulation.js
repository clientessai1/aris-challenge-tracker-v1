...
import { fetchDataFromDbOther, updateChallenges, updateEffectueInChallenges } from './Db/DbManager2';
import DateTimePicker from '@react-native-community/datetimepicker';
import { formatDate, getMyDate, getStrDate } from './helpers/helperFunctions';
import dbManager from './Db/DatabaseManager';
import { useFocusEffect } from '@react-navigation/native';
import { getWeekRange } from './helpers/helperFunctions';

//import dbManager from './Db/DatabaseManager';

const ScreenRecapitulation = ({navigation}) => {
    ...
    const [date_semaine_debut, setDateDebut] = useState(null);
    const [date_semaine_fin, setDateFin] = useState(null);

    const [date_semaine_debut2, setDateDebut2] = useState(null);
    const [date_semaine_fin2, setDateFin2] = useState(null);

    const trieActions = (tab) => {
      //console.log("@@@@@@@@@@@"+tab.length+"@@@@@@@@@@@");
      actions_of_the_week = [];
      tab.forEach(element => {
        
        if (element.jours_action == null)
          return;

        tab_dates = (element.jours_action).split('_');

        if(tab_dates.length > 0){
          nb = tab_dates.length;
          var str_dates = [];
          for(let i=0; i<nb; i++){
            console.log("*******Essai : "+tab_dates[i]);
            if(tab_dates[i] != ""){
              //str_dates.push(tab_dates[i].toString());
              str_dates.push(tab_dates[i].toString()+"!");      
              var une_date = new Date(tab_dates[i]);
              console.log(` ICI : ${une_date} - ${date_semaine_debut2}`);
              if(date_semaine_debut2 <= une_date && une_date <= date_semaine_fin2){
                console.log("ICI2");
                var new_line = {
                  id:element.id, 
                  nom_defi:element.nom_defi,
                  date_debut:element.date_debut,
                  nb_prevu:element.nb_prevu,
                  nb_effectue :element.nb_effectue,
                  jour_action: tab_dates[i].toString()
                };
                actions_of_the_week.push(new_line);
        ...
        console.log("########## - "+actions_of_the_week+" - ###############");
      });
      setData(actions_of_the_week);
      //setData(tab);
    }
    

  useEffect(() => {
    //fetchDataFromDbOther();

    date_exple = new Date('2024-03-19');
    const { monday, sunday } = getWeekRange(/*date_exple*/);
    /*console.log('Monday 2:', getStrDate(getMyDate(monday)));
    console.log('Sunday:', sunday);*/

    setDateDebut(formatDate(monday));
    setDateFin(formatDate(sunday));

    setDateDebut2((monday));
    setDateFin2((sunday));
...
const getBtnStyle = (item) => {
  ...
  return (
    ...
    <View style={my_styles.container3}>        
        {/*<Text style={my_styles.title}>RECAPITULATIF HEBDOMADAIRE</Text>*/}

        <View  style={[my_styles.my_horizontal_alignment, {marginBottom:10,}]}>
        <Text   style={[my_styles.title_shaddow2]} >Du </Text>
      <Text style={[ my_styles.title_shaddow2]} > {date_semaine_debut} </Text>
      <Text style={[my_styles.title_shaddow2]} > au </Text>
      <Text style={[my_styles.title_shaddow2]} > {date_semaine_fin} </Text>
    </View>
    ...
    <View style={my_styles.headerContainer}>
      ...
      <Text style={[my_styles.headerText, {flex:2}]}>Jours</Text>
    </View>
      ...
      {data.map((item, index) => (
        <TouchableOpacity
          ...
          <Text style={[my_styles.itemText, {flex:1.5}]}> {item.nb_effectue} / {item.nb_prevu}</Text>
          <Text style={[my_styles.itemText, {flex:2, margin:0, padding:0}]}>{item.jour_action} </Text>
...
export default ScreenRecapitulation;
